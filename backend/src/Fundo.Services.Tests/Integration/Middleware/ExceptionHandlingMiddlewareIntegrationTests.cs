...
namespace Fundo.Services.Tests.Integration.Middleware
{
    public class ExceptionHandlingMiddlewareIntegrationTests : IClassFixture<WebApplicationFactory<Fundo.Applications.WebApi.Startup>>
    {
        private readonly WebApplicationFactory<Fundo.Applications.WebApi.Startup> _factory;

        public ExceptionHandlingMiddlewareIntegrationTests(WebApplicationFactory<Fundo.Applications.WebApi.Startup> factory)
        {
            _factory = factory;
        }
        ...
        public async Task Middleware_WhenServiceThrowsException_ShouldCatchAndReturn500()
        {
            ...
            errorResponse.Detail.Should().Contain("Database connection failed");
        }
